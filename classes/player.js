class Player {
    constructor(game) {
        ...
        window.addEventListener("keydown", e => {
            ...
            if (this.keys[81] && Number(e.key) > 0) {
                if (Number(e.key) <= game.productsInfo.length)this.tradeSelected = Number(e.key);
            }
            if (e.key === 'c') { //67
                this.use = true;
            }
    ...
    draw(ctx, canvas) {
        ...
        for (let i = 200/10; i >= 0; i--) {
            if (i*10 <= this.lives) ctx.fillStyle = 'red'; else ctx.fillStyle = 'rgb(40, 40, 40, 1)';
        ...
        if (this.viewInventory) {
            ...
            ctx.fillRect(invX, invY, 100, invH);
            let rows = invH / 10;
            let inventory = [];
            ...
            for (let i = 0; i < this.inventory.length; i++) {
                ...
                if (i === this.invSelected) {
                    ...
                    ctx.strokeRect(invX-1, invY+15*i-1, 70, 14);
                    ctx.restore();
                }
            }   
            ctx.restore();
    ...
    blockVerticalCollision (blocks) {
        for (let x = 0; x <= blocks.length-1; x++) {
            for (let y = 0; y <= blocks[x].length-1; y++) {
                ...
                if (block instanceof Tile && collide(this, block)) {
